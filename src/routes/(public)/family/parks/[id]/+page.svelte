    ...
    <div class="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        {#if place.status === 'pending'}
            <div class="flex items-center justify-between gap-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-2xl p-4">
                ...
                {#if currentUserId === place.userId || isAdmin}
                    <a href="/family/parks/{place.id}/edit" class="flex-shrink-0 px-4 py-2 bg-amber-600 text-white rounded-xl text-xs font-black hover:bg-amber-700 transition-all">✏️ Edit</a>
                {/if}
            </div>
        {:else if place.status === 'rejected'}
            <div class="flex items-center justify-between gap-3 text-sm text-red-700 bg-red-50 border border-red-200 rounded-2xl p-4">
                ...
                {#if currentUserId === place.userId || isAdmin}
                    <a href="/family/parks/{place.id}/edit" class="flex-shrink-0 px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-black hover:bg-red-700 transition-all">✏️ Edit &amp; Resubmit</a>
                {/if}
            </div>
        {/if}
